...
class PearTree : public MotherPlant
{
	std::string _name_of_current_tree_class_ = "PearTree";
	size_t  _fruits_remain = 0;

public:
	PearTree(size_t size, Color color, size_t max_fruits) : MotherPlant(size, color, max_fruits) { _fruits_remain = max_fruits; }
